...
people_records.insert(0, ('Denis', 'Vystavnoy', 27, 'QA', 'Kharkiv_region'))
...
for i in range(len(people_records)):
    if i == 6 or i == 10 or i == 13:
        if (people_records[i][2]) >= 30:
            print(f"{people_records[i][0]} дорослiша 30. Її вiк {people_records[i][2]} рокiв.")
        else:
            print(f"{people_records[i][0]} меньше 30. Вiк {people_records[i][2]} рокiв")