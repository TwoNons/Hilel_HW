...
sorted_car = dict(sorted(car_data.items(), key=lambda item: item[1][-1]))
count = 0
for car, data in sorted_car.items():
    if (data[1] == search_criteria[0] or data[2] == search_criteria[1] or data[3] == search_criteria[2]) and count < 5:
        count += 1
        print(car, sorted_car[car])
